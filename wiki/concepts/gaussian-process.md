...
3. **GP の最大の応用先＝ベイズ最適化のサロゲート**（[[bayesian-optimization]] / [[sources/2018-bayesian-optimization-tutorial]]）
   ...
   - 逆に、GP の $O(N^3)$ コストと次元の制約は BayesOpt のボトルネックでもある。**この「GP の代わりに使える高速サロゲート」という空白を、PFN 系が埋めうる**。これを正面から実装したのが **PFNs4BO**（[[sources/2023-pfns4bo]]）で、PFN を GP の「ベイズ的ドロップイン置換」として BO に据え、**固定 GP の事後分布をほぼ厳密に再現**（同論文 図1）しつつ GP では難しい拡張（ユーザー事前分布・無関係次元・非近視眼的獲得関数）を可能にした。高次元 BO の **GIT-BO**（[[sources/2025-git-bo]]）は TabPFNv2 をサロゲートに採用し、TabPFN の順伝播の勾配で能動部分空間を作って GP の $O(N^3)$ 再訓練を回避している（[[tabular-foundation-model]]）。
