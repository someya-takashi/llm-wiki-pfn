...
既知パラメータの**生成モデル**（クラス条件をガウス混合や線形判別で固定）からデータを作ると、各点の**ベイズ最適クラス事後確率 $p(y\mid x)$ が解析的に計算できる**。PFN のクラス確率をこの最適値に突き合わせれば、回帰における GP と同じ「厳密な物差し」が分類で得られる。原典が表形式分類では ECE（較正）止まりだった部分を、**厳密な事後確率比較**にできるのが利点。既知 GMM の場合、この最適事後はガウス混合モデル（[[gaussian-mixture-model]]）の E-step が出す責任 $r_{ic}$ そのもの（[[sources/2024-gmm-em-clustering]]）。**実行可能な手順（データ生成・指標・有限文脈の注意・密度版）は [[questions/evaluating-pfn-with-known-gmm]] に詳述**。
...
閉形式が一切無い任意の prior（BNN など）でも、**十分収束させた NUTS** を真値の代理にして KL/CRPS/NLL で比較できる。厳密ではないが適用範囲が最も広い、最後の手段。MCMC（マルコフ連鎖モンテカルロ）と NUTS の仕組み自体は [[sources/2021-mcmc-explained]]、その収束・一貫性・有効サンプルサイズ（＝「どれだけ回せば物差しとして信頼できるか」）の厳密な議論は [[sources/2019-conceptual-intro-mcmc]]、**NUTS そのものの原典（なぜ turnkey で効率的・不偏な物差しになるか）は [[sources/2014-no-u-turn-sampler]]** を参照。
