...
閉形式が一切無い任意の prior（BNN など）でも、**十分収束させた NUTS** を真値の代理にして KL/CRPS/NLL で比較できる。厳密ではないが適用範囲が最も広い、最後の手段。MCMC（マルコフ連鎖モンテカルロ）と NUTS の仕組み自体は [[sources/2021-mcmc-explained]] を参照。
